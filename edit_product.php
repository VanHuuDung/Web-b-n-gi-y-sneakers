...
$Name = $pro->name;
$Price = $pro->price;
$Gendersid = $pro->gendersId;
$Brandsid = $pro->brandsId;
$PriceDiscount = $pro->priceDiscount;
$ShowOnHomePage = $pro->showOnHomePage;
...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    if(!$NameError && !$DescError && !$PriceError)
    {
        ...
        try {
            ...
            $check = $db->update($Name, $Image, $Gendersid, $Desc, $Price, $PriceDiscount, $ShowOnHomePage, $Brandsid, $id);
            if ($check) {
                unlink("images/<?= $pro->avatar ?>");
...
<?php else:?>
    <?php if($_SESSION['log_detail'] == "0"): ?>
        <h1 style="text-align: center; color: red; padding-top: 170px;">Bạn không có quyền sửa sản phẩm</h1>
    <?php else:?>
        <h2 style="padding-top: 170px;">Chỉnh sửa sản phẩm</h2>
        <?php var_dump($ShowOnHomePage);?>
        <form method="post" enctype="multipart/form-data" class="w-50 m-auto">
            ...
            <div class="mb-3">
                ...
                <input class="form-control" id="des" name="des" value="<?= $Desc ?>"/>
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <input class="form-control" id="pricediscount" name="pricediscount" value="<?= $PriceDiscount?>" />
            </div>
